<template>
  <div class="categoryNav">
    ...
    <div 
      ...
      :key="'category_' + index" 
      :class="{ 'categoryName': true, 'active': index === categoryIndex, 'hidden': index !== categoryIndex }"
      @click="navigateTo(category.to)"
    >
    <span class="ko">
      {{ category.ko }}
    </span>
    ...
    <div 
      ...
      :key="'menu_' + index" 
      :class="{ 'categoryMenu': true, 'active': index === activeIndex, 'hidden': index !== activeIndex }"
    >
      <span class="ko">
        {{  menu.ko  }}
      </span>
...
export default {
  ...
  data() {
    return {
      ...
      { ko: 'IR', en: 'IR', to: '/ir/ir' },
      { ko: '채용정보', en: 'Careers', to: '/careers/recruit' },
      ],
      menus: [
      { ko: '회사개요', en: 'Corporate Overview' },
      { ko: 'CEO메시지', en: 'CEO Message' },
      { ko: '비전', en: 'VISION' },
      { ko: '연혁', en: 'Milestones' },
      { ko: '특허', en: 'Patent' },
      { ko: '수상', en: 'Awards' },
      { ko: '인증', en: 'Certification' },
...
<style scoped>
  ...
  .categoryName{
    @apply text-[#b5b5b6] text-[14px] font-normal leading-[21px] cursor-pointer;
  }

  .categoryMenu{
    @apply text-[#b5b5b6] text-[14px] font-normal leading-[21px];
  }
  
  .categoryMenu.active{
    @apply text-[#141414] font-bold;
  }
</style>